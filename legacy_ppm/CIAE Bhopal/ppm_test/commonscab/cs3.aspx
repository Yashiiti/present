<%@ Page Language="C#" AutoEventWireup="true" CodeFile="cs3.aspx.cs" Inherits="lb3" %>
...
<body>
    ...
    <div>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ppmConnectionString %>"
            SelectCommand="SELECT DISTINCT Question FROM ppm_table WHERE (q_level = 3) AND (dis_id = 6)">
        </asp:SqlDataSource>
        &nbsp; &nbsp;&nbsp;
        ...
        <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ppmConnectionString %>"
            SelectCommand="SELECT [q_option] FROM [ppm_table] WHERE (([dis_id] = 6) AND ([q_level] = 3))">
        </asp:SqlDataSource>
        &nbsp; &nbsp;&nbsp;<br />
        ...
        <asp:Button ID="Button1" runat="server" PostBackUrl="cs2.aspx" Text="Back" /></td>
                <td style="width: 95px; height: 26px">
        <asp:Button ID="Button2" runat="server" PostBackUrl="cs4.aspx" Text="Next" /></td>
                <td style="width: 49px; height: 26px; text-align: right">
                    <asp:Button ID="Button5" runat="server" PostBackUrl="cs_pic_help.aspx" Text="Picture Help" /></td>
                <td style="width: 32px; height: 26px">
                    <asp:Button ID="Button3" runat="server" PostBackUrl="cs_texthelp.aspx" Text="Text Help" /></td>
            </tr>
        </table>
        <br />
        <br />
